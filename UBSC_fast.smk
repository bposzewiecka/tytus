...
, target = 'hg38', query = 'panTro5', window = 5, outgroup = 'rheMac8',  derived = 'target', type= 'target', chromosome = chroms),
	...
	expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/ubcs_fast/derived_in_{derived}/r{region}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.window_size.{window_size}.number_of_bins.{number_of_bins}.rbest.snd.liftover.ubcs.tsv', target = 'hg38', query = ['panTro5'], window = 5, outgroup = 'rheMac8',  derived = ['target', 'query'], type= 'target', chromosome = chroms, window_size = 300, region = 1000 * 1000, number_of_bins = 300)
...
rule generate_ubcs_statistics:
    input:
        liftover_fasta_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.rbest.snd.liftover.fasta'
    output:
        ubcs_stats_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/ubcs_fast/derived_in_{derived}/r{region}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.window_size.{window_size}.number_of_bins.{number_of_bins}.rbest.snd.liftover.ubcs.tsv'
    script:
