...
rule all:
    input:
        ...
        #expand('data/{target}/all/w{window}/outgroup.{outgroup}/all.{target}.all.w{window}.r{region}.outgroup.{outgroup}.rbest.snd.liftover.ubcs.tsv', target = 'hg38', window = 5, outgroup= 'rheMac8', region = 1000 * 1000  ),
        #expand('data/{species}/{species}.hg19_1000g.{chromosome}.snd.ac2.vcf', chromosome =  arch_homini_chroms, species = arch_homini),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.chr{chromosome}.w{window}.outgroup.{outgroup}.vcf.ac2.snd.liftover.fasta', query = arch_homini, target = 'hg19', type = 'target', window = 5, chromosome = arch_homini_chroms, outgroup = 'panTro6'),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.vcf.ac2.snd.liftover.ubcs.tsv', target = 'hg19', query = arch_homini, window = 5, outgroup= 'panTro6', region = 8000 * 1000, type = ['target'], derived = ['target', 'query'], chromosome = hg_chroms),
        #expand('data/{query}/{target}.{query}/{type}/split.log', query = apes, target = 'hg38', type = ['target', 'query']),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.rbest.snd.liftover.ubcs.tsv', target = 'hg38', query = apes, window = 5, outgroup= 'rheMac8', region = 1000 * 1000, type = ['target', 'query'], derived = ['target', 'query'])


        #expand('data/{species}/{species}.hg19_1000g.{chromosome}.snd.ac2.vcf', chromosome =  arch_homini_chroms, species = arch_homini),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.vcf.snd.liftover.fasta', query = 'Denisova', target = 'hg19', type = 'target', window = 5, chromosome = CHROMOSOMES['hg38'], outgroup='panTro6'),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derrived_in_{derived}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.vcf.snd.liftover.ubcs.tsv', query = arch_homini, target = 'hg19', type = 'target', window = 5, chromosome = 'chr2', outgroup='panTro6', derived = 'query', region = 1000 * 1000),
        ...
        #expand_chroms('data/{query}/{target}.{query}/{type}/w{window}/{type}.{target}.{query}.{chromosome}.w{window}.rbest.snd.bed', query = apes, target = 'hg38', window = 5, ttype = 'target' ),
        #expand_chroms('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.rbest.snd.liftover.bed', query = apes, target = 'hg38', window = 5, ttype = 'target', outgroup = 'rheMac8'),
        #expand_chroms('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.rbest.snd.liftover.fasta', query = apes, target = 'hg38', window = 5, ttype = 'target', outgroup = 'rheMac8'),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/{type}.{target}.{query}.chr{chromosome}.w{window}.vcf.snd.bed', query = 'Denisova', target = 'hg19', type = 'target', window = 5, chromosome = arch_homini_chroms),
        #expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.vcf.snd.liftover.bed', query = 'Denisova', target = 'hg19', type = 'target', window = 5, chromosome = CHROMOSOMES['hg38'], outgroup='panTro6')


rule get_UBCS_stats_all_species:
    input:
        lambda wildcards: expand('data/{query}/{target}.{query}/all/w{window}/outgroup.{outgroup}/all.{target}.{query}.w{window}.r{region}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv',  **wildcards, query=apes)
    output:
        'data/{target}/all/w{window}/outgroup.{outgroup}/all.{target}.all.w{window}.r{region}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv'
    shell:
        'cat {input} > {output}'
...
rule get_UBCS_stats_all_types:
    input:
        lambda wildcards: expand('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv', **wildcards, type = ['target', 'query'], derived = ['target', 'query'] )
    output:
        'data/{query}/{target}.{query}/all/w{window}/outgroup.{outgroup}/all.{target}.{query}.w{window}.r{region}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv'
    shell:
        'cat {input} > {output}'

rule get_UBCS_stats_all_chroms:
    input:
        lambda wildcards: expand_chroms('data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv', **wildcards)
    output:
        'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv'
    shell:
        'cat {input} > {output}'

rule get_UBCS_stats:
    input:
        liftover_fasta_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.{file_type}.snd.liftover.fasta',
        chrom_sizes_file = 'data/{target}/{target}.chrom.sizes'
    output:
        ubcs_stats_file =   'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/derived_in_{derived}/r{region}/{type}.{target}.{query}.{chromosome}.w{window}.r{region}.derived_in_{derived}.outgroup.{outgroup}.{file_type}.snd.liftover.ubcs.tsv'
    script:
...
rule create_snd_chrom_file_vcf:
    ...
    output:
        snd_file = 'data/{query}/{target}.{query}/{type}/w{window}/{type}.{target}.{query}.chr{chromosome}.w{window}.vcf.snd.bed'
    script:
        'scripts/python/getSNDsFromVCF.py'

rule get_fasta_from_liftover:
    input:
        bed_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.{file_type}.snd.liftover.bed',
        ref_file = 'data/{outgroup}/{outgroup}.fa'
    output:
        'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.{file_type}.snd.liftover.fasta'
    shell:
        'bedtools getfasta -s -name -fi {input.ref_file} -bed {input.bed_file} > {output}'

rule liftover:
    input:
        SNDs_file = 'data/{query}/{target}.{query}/{type}/w{window}/{type}.{target}.{query}.{chromosome}.w{window}.{file_type}.snd.bed',
        chain_file = lambda wildcards : 'data/{outgroup}/{target}.{outgroup}.rbest.chain'
    output:
        liftover_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.{chromosome}.w{window}.outgroup.{outgroup}.{file_type}.snd.liftover.bed'
    script:
...
rule create_snd_chrom_file_rbest:
    ...
    output:
        snd_file = 'data/{query}/{target}.{query}/{type}/w{window}/{type}.{target}.{query}.{chromosome}.w{window}.rbest.snd.bed'
    script:
...
rule filter_allele_count2:
    input:
        ...
        liftover_fasta_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.chr{chromosome}.w{window}.outgroup.{outgroup}.vcf.snd.liftover.fasta'
    output:
        liftover_fasta_file = 'data/{query}/{target}.{query}/{type}/w{window}/outgroup.{outgroup}/{type}.{target}.{query}.chr{chromosome}.w{window}.outgroup.{outgroup}.vcf.ac2.snd.liftover.fasta'
    shell:
